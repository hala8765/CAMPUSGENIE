...
class MaskedPasswordField(serializers.CharField):
    def __init__(self, **kwargs):
        kwargs['style'] = {'input_type': 'password', 'render_value': True}
        super().__init__(**kwargs)

class UserSerializer(serializers.ModelSerializer):
    name = serializers.CharField(max_length=50)
    email = serializers.CharField(validators=[EmailValidator()])
    password = MaskedPasswordField(write_only=True, required = False)

    class Meta:
        model = User
        fields = ['name', 'email', 'phone', 'password', 'status', 'type']
