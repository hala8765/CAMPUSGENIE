...
os.environ["OPENAI_API_KEY"] = "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"

def addchatAPI(request):
...
def getchatlistAPI(request):
    if request.method == 'GET':
        ...
        if access_token is not None:
            try:
                userId = GetUserIdFromAccessToken(access_token)
                if userId is not None:
                    user = User.objects.get(pk=userId)
...
def GetUserIdFromAccessToken(accessToken):
     try:
        decoded_token = jwt.decode(accessToken, verify=False)  # Assuming you don't need to verify the signature here
...
def chatbotAPI(request, chatId):
    ...
    if request.method == 'POST':
        ...
        if query and chatId and userId:
            ...
            return RetrieveChatHistory(userId, chatId)
        else:
            return JsonResponse({'error': 'Invalid request parameters'})
    else:
        return JsonResponse({'error': 'Invalid request method'})

def RetrieveChatHistory(userId, chatId):
    try:
